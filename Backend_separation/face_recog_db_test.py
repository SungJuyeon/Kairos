...
#
# from face_image_db import get_face_image_from_blob, get_all_user_ids
...
#             best_nickname = "unknown"
#             best_distance = float('inf')
#
#             for user_id in user_ids:
#                 # Get the image from the database
#                 db_image, db_nickname = get_face_image_from_blob(user_id)
#                 if db_image is not None:
#                     # Compare the face_image from the frame with db_image
#                     result = DeepFace.find(face_image, db_path=None, model_name='VGG-Face', enforce_detection=False)
#                     if result['verified']:
#                         print(f"인식된 얼굴의 닉네임: {db_nickname}")
#                         last_detected_nicknames.append(db_nickname)
#                         last_detected_distances.append(result['distance'])
...
#                         if filtered_results:
#                             matched_distance = filtered_results[0]['distance'].values[0]
#                             if matched_distance < best_distance:
#                                 best_distance = matched_distance
#                                 best_nickname = db_nickname
#
#             last_detected_nicknames.append(best_nickname)
#             last_detected_distances.append(best_distance)
#             last_detected_rectangles.append(last_face_positions[idx])  # Add rectangle information
#
...
#                 # 감정 결과 저장 (Neutral 제외)
#                 if emotion != 'neutral':
#                     # 이름이 unknown이 아니면 감정 저장 및 most_emotion_pic 저장
