...
def control_light(state):
    ...
    logging.info(f"light {'ON' if state else 'OFF'}")
    
# induction 제어 함수
def control_induction(state):
    GPIO.output(INDUCTION_PIN, state)
    logging.info(f"induction {'ON' if state else 'OFF'}")    

# 릴레이 제어 함수
def control_relay(state):
    GPIO.output(RELAY_PIN, state)
    logging.info(f"Relay {'ON' if state else 'OFF'}")
